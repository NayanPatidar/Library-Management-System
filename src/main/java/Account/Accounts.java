...
//             System.out.println(" i am called "+all_data_of_Accounts.get(i).Acc_UserName+"  "+all_data_of_Accounts.get(i).Acc_id+"    "+UserName+"   "+id);
            if ((all_data_of_Accounts.get(i).Acc_UserName).equalsIgnoreCase(UserName) && all_data_of_Accounts.get(i).Acc_id == id) {
    ...
    private void Calculate_fine(String UserName, int id) {
        ...
        System.out.println("\n PLEASE WAIT WE ARE CALCULATING YOUR FINE ... \n ");
        for (int i = 0; i < all_data_of_Accounts.size(); i++) {
            if (all_data_of_Accounts.get(i).Acc_UserName == UserName && all_data_of_Accounts.get(i).Acc_id == id) {
                ...
                if (ChronoUnit.DAYS.between(all_data_of_Accounts.get(i).issuDate, currentDate) > 15) {
                    ...
                    // currentDate)));
                    System.out.println(
                            ((ChronoUnit.DAYS.between(all_data_of_Accounts.get(i).issuDate, currentDate)) - 15) * 5);

                    fine_amount = (((ChronoUnit.DAYS.between(all_data_of_Accounts.get(i).issuDate, currentDate)) - 15)
                            * 5);
        ...
        System.out.println("\n CHOOSE OPTION : \n ");
        if(fine_amount!=0)
            System.out.println("\n 1. FOR FINE PAYMENT && RETURNING BOOK \n ");
        System.out.println("\n 2. EXIT \n ");
...
//                Database obj = new Database()
                ...
                cda.issuDate = LocalDate.parse("2023-07-20", dateFormatter);
    ...
    public void Renew_data(int renew_var) throws InterruptedException {
        ...
        try {
            ...
            if(ChronoUnit.DAYS.between(all_data_of_Accounts.get(renew_var).issuDate,LocalDate.now()) < 15){
                ...
                System.out.println("YOUR BOOK IS RENEWED : "+(all_data_of_Accounts.get(renew_var).issuDate = LocalDate.now()));
            }else{
